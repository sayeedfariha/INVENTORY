...
const EmailVerifi = ({ id }) => {
    ...
    const send_email_verification = async () => {
        try {
            ...
            if (verifyCode === email) {
                // ${process.env.NEXT_PUBLIC_API_URL}:5004/send-otp-email
                const responseSendOTP = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}:5004/send-otp/email`, {
                     email,
                    // msg: `Your OTP is ${otp}`,
                ...
                console.log(`responseSendOTP.data`);
                // ${process.env.NEXT_PUBLIC_API_URL}:5004/update/verification_code/${id}
                ...
                // const URL = `http://168.192.0.112:5004/send-otp/13696`;
